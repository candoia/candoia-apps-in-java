...
public class CustomJiraRestClient {
	...
	public final List<b4j.core.Issue> importBugs(String url, String product) throws Exception {
		...
		JiraRpcSession session = new JiraRpcSession();
		...
		if (session.open()) {
			...
			searchData.add("jql",
					"project = HADOOP COMMON AND issuetype = Bug AND status in (Resolved, Closed) AND resolution = Fixed");
	...
	public static void main(String[] args) {
		...
		CustomJiraRestClient b4jimporter = new CustomJiraRestClient();
		String url = "https://issues.apache.org/jira/browse/hadoop";
	...
	public static List<b4j.core.Issue> getIssuesWithBuilder(String url, String product) {
		...
		if (product.equalsIgnoreCase("Tomcat")) {
			for (int i = 1; i < 10; i++) {
				...
				// system.out.println("Total issues from bugzilla : " +
				// issues.size());
