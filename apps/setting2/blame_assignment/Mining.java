package setting2.blame_assignment;
		...
		for (int i = (int) (totalRevs - 1); i > 0; i--) {
			...
			try {
				...
				while (iter.hasNext()) {
					...
					if(entry.getChangedPaths().size() > threashold){
						String name = entry.getAuthor();
						int count = churnDetails.getOrDefault(name, 0)+1;
						churnDetails.put(name, count);
					}
				}
